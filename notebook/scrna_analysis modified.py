...
def preprocess(adata, n_top_genes=3000, target_sum=1e4):
    ...
    sc.pp.scale(ad[:, ad.var['highly_variable']].copy(), max_value=10)
...
sc.tl.rank_genes_groups(adata, groupby='leiden', method='wilcoxon', key_added='rank_genes_wilcox', use_raw=False)
...
### Define `canonical_markers` from PanglaoDB data

The `canonical_markers` dictionary is needed for the `annotate_by_markers` function. It will be constructed from the `markers` DataFrame obtained from PanglaoDB, where 'source' represents the cell type and 'target' represents the Ensembl gene ID.
"""

from collections import defaultdict

canonical_markers = defaultdict(list)

for index, row in markers.iterrows():
    canonical_markers[row['source']].append(row['target'])

# Convert defaultdict to a regular dict
canonical_markers = dict(canonical_markers)

print("First 5 entries of canonical_markers:")
# Displaying a subset for brevity as it can be very large
for i, (cell_type, genes) in enumerate(canonical_markers.items()):
    if i >= 5: # Limit to first 5 for display
        break
    print(f"{cell_type}: {genes[:5]}...") # Show first 5 genes
...
for cell_type, ensembl_ids_from_markers in canonical_markers.items():
    valid_ensembl_ids = []
    for ensembl_id in ensembl_ids_from_markers:
        if ensembl_id in adata.var_names: # Directly check if the Ensembl ID is in adata
            valid_ensembl_ids.append(ensembl_id)
    if valid_ensembl_ids:
        canonical_markers_ensembl[cell_type] = valid_ensembl_ids
...
for ct, genes in canonical_markers_ensembl.items():
    print(f"{ct}: {genes[:5]}...") # Show first 5 genes for brevity
...
expected_ranges = {
    ...
    # Added illustrative ranges for previously undefined cell types
    'Decidual cells': (0.00, 0.001), # Typically not in bone marrow, setting low to flag high
    'T cells naive': (0.05, 0.15), # Illustrative range for T naive cells
    'Gamma delta T cells': (0.01, 0.05), # Illustrative range for Gamma delta T cells
    'Nuocytes': (0.00, 0.02), # Illustrative range for ILC-like cells
    'Alveolar macrophages': (0.00, 0.001), # Typically not in bone marrow, setting low to flag high
    'Parietal cells': (0.00, 0.001), # Typically not in bone marrow, setting low to flag high
    'Trophoblast progenitor cells': (0.00, 0.001), # Typically not in bone marrow, setting low to flag high
    'Pancreatic progenitor cells': (0.00, 0.001), # Typically not in bone marrow, setting low to flag high
    'Delta cells': (0.00, 0.001), # Typically not in bone marrow, setting low to flag high
    'Loop of Henle cells': (0.00, 0.001), # Typically not in bone marrow, setting low to flag high
    'Luteal cells': (0.00, 0.001), # Typically not in bone marrow, setting low to flag high
    'Pluripotent stem cells': (0.00, 0.001) # Setting low to flag detected pluripotency as high
}
...
# The variable `ann_df` was not defined. Please ensure previous cells defining `ann_df` are executed.
# For now, we will use the first group from the `celltype_suggested` categories as an example.
# cluster_of_interest = list(ann_df['cluster'])[0]  # Original line causing NameError

sc.tl.rank_genes_groups(adata, groupby='celltype_suggested', method='wilcoxon', key_added='rank_genes_celltype', use_raw=False)
...
# Use the first group for the example, as `cluster_of_interest` was intended as an example
cluster_of_interest = groups[0]
for g in groups:
...
# Convert gene symbols to uppercase to match gene set database expectations
...
# Added min_size=1 to be less restrictive for demonstration, adjust as needed.
prerank_res = gp.prerank(rnk= str(Path(DRIVE_SAVE_DIR)/f"{group}_pre_rank.rnk"),
                         gene_sets='KEGG_2016', outdir=DRIVE_SAVE_DIR, permutation_num=100, min_size=1)
